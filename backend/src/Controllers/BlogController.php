...
class BlogController {
    ...
    public function create($data, $currentUser) {
        ...
        // Validate slideshow_images if provided
        if (isset($data['slideshow_images'])) {
            if (!is_array($data['slideshow_images'])) {
                Response::error("slideshow_images must be an array", 400);
            }
            if (count($data['slideshow_images']) > 5) {
                Response::error("Maximum 5 slideshow images allowed", 400);
            }
        }
        ...
        $data['slideshow_images'] = $data['slideshow_images'] ?? [];
