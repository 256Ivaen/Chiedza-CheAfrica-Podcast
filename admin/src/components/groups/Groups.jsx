...
const GroupsWithWebSocket = () => {
  ...
  useEffect(() => {
    const loadPersistedState = () => {
      try {
        // Load selected group
        const cachedSelectedGroup = localStorage.getItem(
          "socialGems_selectedGroup"
        );
        ...
        const cachedShowChatList = localStorage.getItem(
          "socialGems_showChatList"
        );
        ...
        const cachedHiddenDeleted = localStorage.getItem(
          "socialGems_hiddenDeletedMessages"
        );
  ...
  useEffect(() => {
    if (hiddenDeletedMessages.size > 0) {
      localStorage.setItem(
        "socialGems_hiddenDeletedMessages",
        JSON.stringify([...hiddenDeletedMessages])
      );
    }
      ...
      if (data && data.length > 0) {
        ...
        if (currentGroupIds !== newGroupIds) {
          ...
          const newGroups = data.filter(
            (newGroup) =>
              !groups.find(
                (existingGroup) => existingGroup.group_id === newGroup.group_id
              )
          );
        ...
        try {
          ...
          if (messagesData && messagesData.length > 0) {
            ...
            if (existingIds !== newIds) {
              console.log(
                `📨 [TanStack] Messages updated for group ${group.group_id}`
              );
          ...
          console.error(
            `Background message refresh failed for group ${group.group_id}:`,
            error
          );
  ...
  const addMessageToGroup = useCallback(
    (groupId, message) => {
      console.log(
        "📝 [Groups] Adding message to group:",
        groupId,
        "MessageID:",
        message.messageId,
        "From:",
      ...
      setGroupMessagesMap((prevMap) => {
        ...
        if (messageExists) {
          console.log(
            "⚠️ [Groups] Duplicate message skipped:",
            message.messageId
          );
          return prevMap;
        }
        ...
        console.log(
          "✅ [Groups] Message added. Total messages in group:",
  ...
  useEffect(() => {
    ...
    const removeListener = addMessageListener((data) => {
      ...
      if (messageTypes.includes(data.type) && data.message) {
        ...
        console.log(
          "💬 [Groups] Processing message for group:",
          groupId,
          "From user:",
          message.senderId
        );
        ...
        if (
          ...
          selectedGroup?.group_id !== groupId
        ) {
      ...
      if (
        ...
        console.log(
          "🗑️ [Groups] Marking message as deleted:",
          data.messageId,
          "from group:",
          groupId
        );
        ...
        if (deletedMessage) {
          ...
          setLastMessages((prevMap) => {
            ...
            if (
              currentLastMessage &&
              currentLastMessage.messageId === data.messageId
            ) {
      ...
      if (data.type === "MESSAGE_EDITED" && data.message) {
        ...
        setLastMessages((prevMap) => {
          ...
          if (
            currentLastMessage &&
            currentLastMessage.messageId === message.messageId
          ) {
            newMap.set(groupId, message);
          }
          return newMap;
  ...
  const fetchGroups = async () => {
    try {
      ...
      if (response?.data && Array.isArray(response.data)) {
        ...
        console.log(
          "📂 [Groups] Loading messages for all groups immediately..."
        );
    ...
    try {
      ...
      if (uploadResponse?.status === 200 && uploadResponse?.data) {
        ...
        if (Array.isArray(uploadResponse.data)) {
          mediaUrl =
            uploadResponse.data[0]?.file_url || uploadResponse.data[0]?.url;
  ...
  const handleSendMessage = async (e) => {
    ...
    if (isEditMode) {
      try {
        ...
        setLastMessages((prevMap) => {
          ...
          if (
            currentLastMessage &&
            currentLastMessage.messageId === editingMessage.messageId
          ) {
    ...
    try {
      ...
      if (response?.data) {
        ...
        console.log(
          "✅ [Groups] Message sent successfully, broadcasting via WebSocket..."
        );
        sendMessage({
          type: "SEND_MESSAGE",
          message: realMessage,
      ...
      setMessages((prevMessages) =>
        prevMessages.filter((msg) => msg.messageId !== tempId)
      );
  ...
  const renderMessages = () => {
    ...
    messages.forEach((message, index) => {
      ...
      if (
        ...
        hiddenDeletedMessages.has(message.messageId)
      ) {
        return;
      }
  ...
  useEffect(() => {
    if (selectedGroup && groupMessagesMap.has(selectedGroup.group_id)) {
      const existingMessages =
        groupMessagesMap.get(selectedGroup.group_id) || [];
      setMessages(existingMessages);
      setTimeout(() => {
        messagesEndRef.current?.scrollIntoView({ behavior: "instant" });
      }, 100);
  ...
  return (
    <div className="flex-1 flex relative overflow-hidden">
      ...
      <div
        className={cn(
          "bg-white border-r border-primary flex flex-col transition-all duration-300",
          isMobile ? (showChatList ? "w-full" : "hidden") : "w-80"
        )}
      >
        <div className="p-4 bg-gradient-primary-soft text-secondary border-b border-secondary/20">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <h1 className="text-sm uppercase font-semibold">Social Gems Chat</h1>
              <ConnectionStatus
          ...
          <div className="relative">
            <FiSearch className="absolute left-3 top-1/2 transform -translate-y-1/2 text-secondary w-4 h-4" />
            <input
              type="text"
              placeholder="Search groups..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-10 pr-4 py-2 bg-white border border-secondary/60 rounded-full focus:outline-none focus:ring-2 focus:ring-primary/50 text-xs text-secondary placeholder-secondary/80"
            />
          </div>
        </div>
        ...
        {selectedGroup ? (
          <>
            ...
            >
              {messagesLoading ? (
                ...
                getTypingIndicator(selectedGroup?.group_id) ? (
                <>
                  {renderMessages()}
          ...
          <div className="flex-1 flex items-center justify-center bg-white">
            <div className="text-center">
              ...
              <h3 className="text-sm uppercase font-semibold text-secondary mb-2">
                Social Gems Chat
              </h3>
              <p className="text-xs text-secondary/70 max-w-md">
                Connect with your team and collaborate seamlessly.
                <br />
